          ...
          case 'web':
            ...
            if (this.grant_types.indexOf('implicit') !== -1 && redirectUri.startsWith('http:')) {
              if(this.allow_implicit_insecure_redirect) {
                provider.emit('implicit_insecure_redirect.warning', `Allowing http redirect uri '${redirectUri}' for client ${this.client_name} (${this.client_id}) that has implicit grant enabled.`, this);                
              } else {
            ...
            if (this.grant_types.indexOf('implicit') !== -1 && url.parse(redirectUri).hostname === 'localhost') {
              if(this.allow_implicit_insecure_redirect) {
                provider.emit('implicit_insecure_redirect.warning', `Allowing localhost redirect uri '${redirectUri}' for client ${this.client_name} (${this.client_id}) that has implicit grant enabled.`, this);                
              } else {
