...
const RECOGNIZED_METADATA = [
  'allow_implicit_insecure_redirect',
  'application_type',
        ...
        switch (this.application_type) { // eslint-disable-line default-case
          case 'web':
            ...
            if (this.grant_types.indexOf('implicit') !== -1 && redirectUri.startsWith('http:')) {
              if(this.allow_implicit_insecure_redirect) {
                // console.warn(`Allowing insecure redirect uri for client ${this.client_name} (${this.client_id}) that has implicit grant enabled.`)
                provider.emit('warning', `Allowing http redirect uri '${redirectUri}' for client ${this.client_name} (${this.client_id}) that has implicit grant enabled.`, this);                
              } else {
            ...
            if (this.grant_types.indexOf('implicit') !== -1 && url.parse(redirectUri).hostname === 'localhost') {
              if(this.allow_implicit_insecure_redirect) {
                // console.warn(`Allowing insecure redirect uri for client ${this.client_name} (${this.client_id}) that has implicit grant enabled.`)
                provider.emit('warning', `Allowing localhost redirect uri '${redirectUri}' for client ${this.client_name} (${this.client_id}) that has implicit grant enabled.`, this);                
              } else {
