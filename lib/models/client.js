  ...
  class Client {
    ...
    backchannelLogout(sub, sid) {
      ...
      return logoutToken.sign(this, { noExp: true })
        ...
        .then(({statusCode, headers, body, url}) => {
          console.log(`Called backchannel logout URI ${this.backchannelLogoutUri} for sub ${sub}. response: ` + JSON.stringify({statusCode, headers, body, url}));
        })
        .catch(error => {
          console.error(`Error calling backchannel logout URI ${this.backchannelLogoutUri} for sub ${sub}: ${error}`);
          // swallow
        }));
    }
