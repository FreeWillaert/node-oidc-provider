...
module.exports = function getClientCredentials(provider) {
  ...
  return class ClientCredentials extends BaseToken {
    ...
    // TODO: Make more DRY with access_token ?
    ...
    sign(client, options) {
      ...
      if (getCustomAccessTokenClaimsFunc) {
        ...
        const accountId = null; // No accountid available for client credentials grant.
        atClaimsPromise = Promise.resolve(getCustomAccessTokenClaimsFunc(client.clientId, requestedAudiences, requestedScopes, this.jti, accountId, provider.issuer, options.availableClaims, timestamp, expiresIn));
      }
      ...
      .then(result => {
        ...
        let atClaims = result || {
          ...
          iss: provider.issuer,
          // sub is not available for client_credentials
        ...
        return (() => {
          ...
          if (alg) {
            ...
            return JWT.sign(payload, key, alg, { /* All claims are already set in payload. */ });
          }
