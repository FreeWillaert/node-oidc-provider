...
module.exports.handler = function getAuthorizationCodeHandler(provider) {
  ...
  return function* authorizationCodeResponse(next) {
    ...
    if (jwtat) {
      // For auth code grant, we work on the /token request, through which no scope param is passed; thus, this.oidc.params.scope must be set based on the value previously stored when handling the /auth request.
      this.oidc.params.scope = code.scope;
      accessToken = yield at.sign(this.oidc.client, { params: this.oidc.params, audience: code.audience, availableClaims: yield Promise.resolve(account.claims())});
    }
