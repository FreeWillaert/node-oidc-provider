...
module.exports.handler = function getRefreshTokenHandler(provider) {
  return function* refreshTokenResponse(next) {
    ...
    // TODO: TEST JWTAT WITH REFRESH TOKEN PROPERLY
    // const accessToken = yield at.save();
    // if enabled, return JWT access token
    const accessTokenJwt = yield at.sign(this.oidc.client, { params: this.oidc.params });
    let accessToken = yield at.save(); // TODO: Need to change anything about at.save?

    const jwtat = instance(provider).configuration('features.jwtat');
    accessToken = jwtat ? accessTokenJwt : accessToken;
