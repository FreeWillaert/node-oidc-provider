...
module.exports.handler = function getRefreshTokenHandler(provider) {
  return function* refreshTokenResponse(next) {
    ...
    if (this.oidc.params.scope) {
      if (instance(provider).configuration('features.enableAdditionalScopesOnRefresh')) {
        const authorizedScopes = yield Promise.resolve(instance(provider).configuration().authorizeRefreshTokenScopes(refreshToken.accountId, refreshToken.clientId, refreshToken.audience, refreshTokenScopes, this.oidc.params.scope.split(' ')))
        this.oidc.params.scope = authorizedScopes.join(' ');
      }
      else {
    ...
    let accessToken = yield at.save();
    const jwtat = instance(provider).configuration('features.jwtat');

    if (jwtat) {
      // For refresh token grant, the scope param is optional; thus, if not present, this.oidc.params.scope must be set based on the value previously stored with the refresh token.
      this.oidc.params.scope = this.oidc.params.scope || refreshToken.scope;

      accessToken = yield at.sign(this.oidc.client, { params: this.oidc.params, audience: refreshToken.audience, availableClaims: yield Promise.resolve(account.claims()) });
    }
