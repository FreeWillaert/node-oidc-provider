  ...
  return {
    ...
    post: compose([
      ...
      function* endSession(next) {
        ...
        if (this.oidc.params.logout) {
          ...
          if (backchannelLogout) {
            try {
              ...
              const logouts = clientIds.map(visitedClientId => Client.find(visitedClientId)
                .then((visitedClient) => {
                  if (visitedClient && visitedClient.backchannelLogoutUri) {
                    endSessionInfo.backchannelLogoutUris = endSessionInfo.backchannelLogoutUris || [];
                    endSessionInfo.backchannelLogoutUris.push({ clientId: visitedClientId, logoutUri: visitedClient.backchannelLogoutUri });
                    return visitedClient.backchannelLogout(this.oidc.session.accountId(),
                      (backchannelLogout.sidPerSession === true) ? this.oidc.session.id : this.oidc.session.sidFor(visitedClient.clientId));
                  }
                  return undefined;
                }));
              ...
              endSessionInfo.backchannelLogoutError = (err.message || "[unknown error]") + (err.stack ? "-" + err.stack : "");
            }
          }
        ...
        provider.emit('end_session.success', endSessionInfo, this);
        this.redirect(uri);
