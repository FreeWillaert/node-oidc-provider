...
module.exports = (provider) => {
  ...
  return function* interactions(next) {
    ...
    if (interaction) {
      ...
      const promptedNoneOriginal = ctx.prompted('none'); // ISSUE: since there is already a ctx.result, the prompted function will *always* return true here
      const promptedNone = ctx.prompts && ctx.prompts.indexOf('none') !== -1;

      this.assert(!promptedNone, 302, interaction.error, {
