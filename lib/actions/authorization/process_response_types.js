...
module.exports = (provider) => {
  ...
  function* tokenHandler() {
    const at = new AccessToken({
      ...
      sid: (backchannelLogout.sidPerSession === true) ? this.oidc.session.id : this.oidc.session.sidFor(this.oidc.client.clientId),
    });
    ...
    if (jwtat) {
      accessToken = yield at.sign(this.oidc.client, { params: this.oidc.params, audience: this.oidc.params.audience, availableClaims: yield Promise.resolve(this.oidc.account.claims())});
    }
  ...
  function* codeHandler() {
    const ac = new AuthorizationCode({
      ...
      acr: this.oidc.acr,
      amr: this.oidc.amr,
      audience: this.oidc.params.audience,
      ...
      scope: this.oidc.params.scope,
      state: this.oidc.params.state // Support FAPI s_hash
    });
    ...
    if (backchannelLogout) {
      ac.sid = (backchannelLogout.sidPerSession === true) ? this.oidc.session.id : this.oidc.session.sidFor(this.oidc.client.clientId);
    }
  ...
  function* idTokenHandler() {
    ...
    if (backchannelLogout) {
      token.set('sid', (backchannelLogout.sidPerSession === true) ? this.oidc.session.id : this.oidc.session.sidFor(this.oidc.client.clientId))
    }
